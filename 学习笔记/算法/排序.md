...
我真的很痛苦，写个贪心，就搞这个冒泡排序我能搞一小时。第一先模拟好，把一切都模拟好，把每个细节想好再去写算法，不要可能是这样，不要随便就上手写，容易死

```
 public int[] ab(int[] a){
        int length=a.length;
        for(int i=0;i<length-1;i++){
            for(int j=0;j<length-1-i;j++){  //我就卡在这里，你第二次遍历的方向不一样，那么最大或最小的数在的位置就不一样，左往右，数就在最右边。
                if(a[j]>a[j+1]){
                    int tem=a[j];
                    a[j]=a[j+1];
                    a[j+1]=tem;
...
//记住几个关键信息，交换不用记，两个for循环是重点，第一个for循环就是length-1，第二个for循环从左往右所以j=0，每次交换都要少一个所以是length-1-i，每次大循环都排序好了一个数
```
